...
% 
% p_std_list = 0.1:0.05:0.5;
% max_iter = 10;
% 
% % generate task difficulty
% error_mv = zeros(size(p_std_list, 2), max_iter, 3);
% error_alg1 = zeros(size(p_std_list, 2), max_iter, 3);
% error_alg2 = zeros(size(p_std_list, 2), max_iter, 3);
% error_oracle = zeros(size(p_std_list, 2), max_iter, 3);
% error_mv_em = zeros(size(p_std_list, 2), max_iter, 3);
% error_sm_em = zeros(size(p_std_list, 2), max_iter, 3);
% error_pgd = zeros(size(p_std_list, 2), max_iter, 3);
% 
% for p_idx = 1 : length(p_std_list)
%     p_std = p_std_list(p_idx);
%     p_min = 0.501 - p_std;
%     p_max = 0.499 + p_std;
% 
%     
...
for plot_idx = 1 : 3
    ...
    title(title_list(plot_idx), 'Interpreter','latex');
    xlabel('Std. of worker reliability','fontsize',26);
    ylabel('Prediction error', 'fontsize', 26)
    xlim([x(1) x(end)]);
    set(gca,'FontSize', 16)
