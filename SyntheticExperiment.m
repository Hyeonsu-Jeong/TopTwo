...
error_mv = zeros(size(sampling_list, 2), max_iter, 3);
error_alg1 = zeros(size(sampling_list, 2), max_iter, 3);
error_alg2 = zeros(size(sampling_list, 2), max_iter, 3);
error_oracle = zeros(size(sampling_list, 2), max_iter, 3);
error_mv_em = zeros(size(sampling_list, 2), max_iter, 3);
error_sm_em = zeros(size(sampling_list, 2), max_iter, 3);
error_pgd = zeros(size(sampling_list, 2), max_iter, 3);
...
%         fprintf("%d %d\n", sampling_idx, times);
        ...
        [g, h] = top2(q);
        error_mv_em(sampling_idx, times, :) = 1 - grade(trimmed_ground_truth, trimmed_most_confusing_answer, g, h) / total_task;
...
%         
%                 [g, h] = top2(q);
%                  error_sm_em(sampling_idx, times, :) = 1 - grade(trimmed_ground_truth, trimmed_most_confusing_answer, g, h) / total_task;
        %====================== PGD ==========================================
        ...
        [g, h] = top2(score);
        error_pgd(sampling_idx, times, :) = 1 - grade(trimmed_ground_truth, trimmed_most_confusing_answer, g, h) / total_task;
        % ================================ TopTwo1 ================================
        [N, M] = size(Y_obs);
        K = max(Y_obs, [], 'all');
        ...
        [g, h] = top2(-del_w);
        error_alg1(sampling_idx, times, :) = 1 - grade(trimmed_ground_truth, trimmed_most_confusing_answer, g, h) / total_task;
        % ================================ TopTwo2 ================================
        ...
        error_alg2(sampling_idx, times, :) = 1 - grade(trimmed_ground_truth, trimmed_most_confusing_answer, g, h) / total_task;
        % ================================ Oracle ================================
        ...
        error_oracle(sampling_idx, times, :) = 1 - grade(trimmed_ground_truth, trimmed_most_confusing_answer, g, h) / total_task;
        % ================================ MV =====================================
        ...
        error_mv(sampling_idx, times, :) = 1 - grade(trimmed_ground_truth, trimmed_most_confusing_answer, g, h) / total_task;
    end
end
...
x =  total_worker * sampling_list;

fig = figure(1);
set(fig, 'OuterPosition', [100, 200, 1600, 600])
...
for plot_idx = 1 : 3
    ...
    for i = 1 : 7
        A(i) = plot(x(2:end), error_mean(i, 2:end), 'Color', color_list{i}, 'LineStyle', '-',...
            'LineWidth', 1.5, 'Marker', marker_list{i},  'MarkerFaceColor', color_list{i},'MarkerSize',5);
        hold on
    end
    
    for i = 1 : 7
        xforfill = [x(2:end), fliplr(x(2:end))];
        yforfill = [error_lb(i, 2:end), fliplr(error_ub(i, 2:end:))];
    ...
    xlabel('Avg, # of queries per task','fontsize',26);
