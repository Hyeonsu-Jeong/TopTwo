...
q_std_list = 0.05:0.025:0.25;
max_iter = 10;

% generate task difficulty
error_mv = zeros(size(q_std_list, 2), max_iter, 3);
error_alg1 = zeros(size(q_std_list, 2), max_iter, 3);
error_alg2 = zeros(size(q_std_list, 2), max_iter, 3);
error_oracle = zeros(size(q_std_list, 2), max_iter, 3);
error_mv_em = zeros(size(q_std_list, 2), max_iter, 3);
error_sm_em = zeros(size(q_std_list, 2), max_iter, 3);
error_pgd = zeros(size(q_std_list, 2), max_iter, 3);

for q_idx = 1 : length(q_std_list)
    q_std = q_std_list(q_idx);
    q_min = 0.751 - q_std;
    q_max = 0.749 + q_std;
    ...
    for times = 1:max_iter
        ...
        [g, h] = top2(q);
        error_mv_em(q_idx, times, :) = 1 - grade(trimmed_ground_truth, trimmed_most_confusing_answer, g, h) / total_task;
        ...
        [g, h] = top2(score);
        error_pgd(q_idx, times, :) = 1 - grade(trimmed_ground_truth, trimmed_most_confusing_answer, g, h) / total_task;
        % ================================ TopTwo1 ================================
        [N, M] = size(Y_obs);
        K = max(Y_obs, [], 'all');
        ...
        [g, h] = top2(-del_w);
        error_alg1(q_idx, times, :) = 1 - grade(trimmed_ground_truth, trimmed_most_confusing_answer, g, h) / total_task;
        % ================================ TopTwo2 ================================
        ...
        error_alg2(q_idx, times, :) = 1 - grade(trimmed_ground_truth, trimmed_most_confusing_answer, g, h) / total_task;
        % ================================ Oracle ================================
        ...
        error_oracle(q_idx, times, :) = 1 - grade(trimmed_ground_truth, trimmed_most_confusing_answer, g, h) / total_task;
        % ================================ MV =====================================
        ...
        error_mv(q_idx, times, :) = 1 - grade(trimmed_ground_truth, trimmed_most_confusing_answer, g, h) / total_task;
    end
end
...
for plot_idx = 1 : 3
    ...
    title(title_list(plot_idx), 'Interpreter','latex');
    xlabel('Std of task difficulty','fontsize',26);
    ylabel('Prediction error', 'fontsize', 26)
    xlim([x(1) x(end)]);
    set(gca,'FontSize', 16)
